...
```python
@staticmethod
def get_tags(template_id: str, **opts: Unpack[ApiParams]) -> List[TemplateTag]
```
...
List of TemplateTag with tag name, build_id, and created_at
Example
```python
from e2b import Template
...
```python
def get_template_tags(client: AuthenticatedClient,
